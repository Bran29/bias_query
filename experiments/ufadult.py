...
def Adult_UEffect(K_max):
    ...
    for k in range(K_max):
        ...
        for ii in range(5):
            ...
            y_pred_z1 = y_pred[np.where(D_provider_x[ii][:, -5] > 0)[0]]
            y_pred_z0 = y_pred[np.where(D_provider_x[ii][:, -5] <= 0)[0]]
            Uf = abs(
                np.count_nonzero(y_pred_z1 == 1)
                / np.count_nonzero(D_provider_x[ii][:, -5] > 0)
    ...
    plt.legend()
    plt.savefig("fig/syn_uf_effect.jpg")
