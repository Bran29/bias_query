...
def Adult_UEffect(K_max):
    ...
    (
        ...
        D_test_x,
        D_test_y,
    ) = generate_bank_sample(D_init_num=1, D_providers_num=5)
    D_init_y=add_noise(D_init_y,0.5)
    ...
    for k in range(K_max):
        ...
        for ii in range(len(D_provider_y)):
            D_provider_y[ii]=add_noise(D_provider_y[ii],ii*0.1)
    ...
    plt.legend()
    plt.savefig("fig/BANK_ua_effect.jpg")
